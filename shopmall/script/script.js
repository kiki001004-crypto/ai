...
function renderCategoryProducts(filter = "all") {
  ...
  const filtered =
    filter === "all"
      ? categoryProducts
      : categoryProducts.filter((item) => item.category === filter);
  categoryWrap.innerHTML = filtered.map(productRow).join("");
  if (titleEl)
    titleEl.textContent = filter === "all" ? "전체 상품" : `${filter} 상품`;
  ...
  const activeChip = document.querySelector(
    `#categoryTabs .filter-chip[data-filter="${filter}"]`,
  );
  if (activeChip) {
...
function setupCategoryTabs() {
  ...
  tabsWrap.addEventListener("click", (event) => {
    ...
    const hash =
      Object.keys(hashMap).find((key) => hashMap[key] === filter) || "#all";
...
function renderDetailPage() {
  ...
  const item =
    allProducts.find((product) => product.id === id) || allProducts[0];
  ...
  document
    ...
    ?.addEventListener("click", () => syncSlider(currentIndex - 1));
  document
    .querySelector(".slider-arrow.next")
    ?.addEventListener("click", () => syncSlider(currentIndex + 1));
  document
    .getElementById("detailThumbs")
  ...
  document.getElementById("sizeOptions")?.addEventListener("click", (event) => {
    ...
    sizeButtons.forEach((sizeBtn) =>
      sizeBtn.classList.toggle("active", sizeBtn === button),
    );
  });
...
function renderCartPage() {
  ...
  if (!items.length) {
    cartWrap.innerHTML =
      '<div class="empty-cart">장바구니가 비어 있어요.<br />마음에 드는 상품을 담아보세요.</div>';
    renderSummary(items);
    return;
  }
...
if (recommendedWrap)
  recommendedWrap.innerHTML = recommendedProducts.map(productCard).join("");
