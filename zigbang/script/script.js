...
const loadingPercentEl = document.querySelector(".splash-loading-percent");
...
// 메인 로고 클릭 시 최상단 스크롤
// ==========================================
const mainLogos = document.querySelectorAll(".logo-small, .main-logo-img");
mainLogos.forEach((logo) => {
  logo.addEventListener("click", (e) => {
    if (!logo.classList.contains("page-link")) {
      const parentView = e.target.closest(".app-view");
      if (parentView) {
        const scrollContent = parentView.querySelector(".main-scroll-content");
        if (scrollContent)
...
const mapKeywordCenters = [
  ...
  {
    keywords: ["수원", "수원시", "수원역", "광교", "광교역", "영통", "영통역"],
    lat: 37.2636,
    lng: 127.0286,
    level: 7,
  },
  ...
  {
    keywords: ["안양", "안양시", "안양역", "평촌", "평촌역", "범계", "범계역"],
    lat: 37.3943,
    lng: 126.9568,
    level: 6,
  },
  ...
  {
    keywords: ["과천", "과천시", "과천역", "정부과천청사", "정부과천청사역"],
    lat: 37.4292,
    lng: 126.9877,
    level: 6,
  },
  ...
  {
    keywords: ["의정부", "의정부시", "의정부역", "양주", "양주시", "양주역"],
    lat: 37.7381,
    lng: 127.0337,
    level: 7,
  },
...
const openFilterModal = () => {
  ...
  const activeStage = document.querySelector(
    ".app-view.is-active .design-stage",
  );
...
const closeFilterModal = () => {
  ...
  const activeHash =
    activeView && activeView.id ? `#${activeView.id}` : "#search-view";
...
if (filterApplyBtn) {
  filterApplyBtn.addEventListener("click", () => {
    if (typeof currentFilters !== "undefined") {
      currentFilters.pet = !!filterModal?.querySelector(
        '[data-map-filter="pet"].is-selected',
      );
      currentFilters.subway = !!filterModal?.querySelector(
        '[data-map-filter="subway"].is-selected',
      );
      currentFilters.fee = !!filterModal?.querySelector(
        '[data-map-filter="fee"].is-selected',
      );
      renderMapOverlays();
    }
    closeFilterModal();
...
function findPropertySearchResults(query) {
  ...
  const matchedItems = propertySearchData.filter((item) => {
    ...
    return (
      ...
      normalizedQuery.includes(item.name.replace(/\s+/g, "").toLowerCase())
    );
  });
...
const propertyResultList = document.getElementById("property-result-list");
const btnClearPropertyResult = document.getElementById(
  "btn-clear-property-result",
);
const propertyDetailPopup = document.getElementById("property-detail-popup");
const propertyDetailClose = document.getElementById("property-detail-close");
...
const propertyDetailPrice = document.getElementById("property-detail-price");
const propertyDetailArea = document.getElementById("property-detail-area");
...
const propertyDetailChecks = document.getElementById("property-detail-checks");
const propertyDetailFavBtn = document.getElementById("property-detail-fav-btn");
...
function isFavoriteProperty(propertyName) {
  return (
    typeof favoriteNames !== "undefined" && favoriteNames.includes(propertyName)
  );
}
...
function openPropertyDetailPopup(item) {
  ...
  if (propertyDetailStation)
    propertyDetailStation.textContent = item.station || "교통 정보 확인 필요";
  ...
  propSearchInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      ...
      if (val) {
        ...
        document.querySelectorAll(".rank-item").forEach((rankItem) => {
          rankItem.addEventListener("click", () => {
            const keyword = rankItem
              .querySelector(".rank-name")
              ?.textContent?.trim();
            if (!keyword || !propSearchInput) return;
            propSearchInput.value = keyword;
            renderPropertySearchResults(keyword, true);
...
function saveFavoriteProperties() {
  localStorage.setItem(FAVORITE_PROPERTIES_KEY, JSON.stringify(favoriteNames));
}
...
function bindFavoriteSelection() {
  getFavoriteInputs().forEach((input) => {
    input.addEventListener("change", () => {
      ...
      const checkedFavorites = favoriteInputs.filter((item) => item.checked);
...
function getCompareShareText() {
  ...
  const rightStation =
    document
      ...
      ?.innerText.replace(/\n+/g, " ") || "";
  const leftFee = document.getElementById("compare-left-fee")?.innerText || "";
  const rightFee =
...
const favoriteCompareBtn = document.querySelector(".favorite-compare-btn");
...
const toggleFavoriteMenuPanel = () => {
  ...
  favoriteMenuPanel.setAttribute("aria-hidden", willOpen ? "false" : "true");
  if (favoriteMenuBtn) {
    favoriteMenuBtn.setAttribute("aria-expanded", willOpen ? "true" : "false");
  }
};
...
document.querySelectorAll(".my-save-alert-btn").forEach((btn) => {
  btn.addEventListener("click", () => showToast("알림 설정을 저장했습니다."));
});
...
const myNewReservationBtn = document.getElementById("my-new-reservation-btn");
...
const myScheduleTimeList = document.getElementById("my-schedule-time-list");
...
if (myReservationList) {
  myReservationList.addEventListener("click", (e) => {
    ...
    if (cancelBtn && card) {
      const title = card.querySelector("strong")?.textContent || "방문 예약";
      card.remove();
...
if (propertyDetailAlertBtn) {
  propertyDetailAlertBtn.addEventListener("click", () => {
    ...
    propertyDetailAlertBtn.textContent = isActive
      ? "알림 설정완료"
      : "알림 받기";
    showToast(
      isActive
...
const showView = (targetSelector) => {
  ...
  if (typeof swiper !== "undefined" && targetSelector !== "#walkthrough") {
